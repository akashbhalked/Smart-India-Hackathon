...
session_start();
  ...
  $query = $connection->prepare("SELECT empid  FROM radmin WHERE uid=?");  //yaha userid se check krna hai....
  $query->bind_param('s',$user );
      ...
      $numRows = $query->affected_rows;
          if($numRows>0)
              {
                $rand=rand(1000,9999);
              $query1 = $connection->prepare("UPDATE radmin SET  OTP = ? WHERE uid=?");
              ...
              if($numRows>0)
              {
                header("Location: iearailotp.php?mob=$mobileno");
              }
            }/*if($numRows>0)
